...
class DatabaseMetricsCollector:
    ...
    def _init_connections(self):
        ...
        try:
            ...
            self.connections['mongodb'] = pymongo.MongoClient(
                'mongodb://dbcostops_monitor:monitor_password@localhost:27017/dbcostops_monitoring?authSource=admin'
            )
        ...
        try:
            ...
            queries = {
                'connections': {
                    ...
                    'active_queries': "SELECT sum(CASE WHEN state = 'active' THEN 1 ELSE 0 END) FROM pg_stat_activity WHERE state = 'active'"
                },
                'database_size': {
                    'size_bytes': "SELECT pg_database_size('dbcostops_monitoring')",
                    'size_pretty': "SELECT pg_size_pretty(pg_database_size('dbcostops_monitoring'))"
                },
                'query_performance': {
                    'avg_query_time': "SELECT avg(mean_exec_time) FROM pg_stat_statements WHERE calls > 0",
                    'max_query_time': "SELECT max(max_exec_time) FROM pg_stat_statements WHERE calls > 0",
                    'total_calls': "SELECT sum(calls) FROM pg_stat_statements WHERE calls > 0",
                    'total_exec_time': "SELECT sum(total_exec_time) FROM pg_stat_statements WHERE calls > 0"
                }
            }
            ...
            for name, query_data in queries.items():
                try:
                    if isinstance(query_data, dict):
                        metrics[name] = {}
                        for sub_name, sub_query in query_data.items():
                            cursor.execute(sub_query)
                            result = cursor.fetchone()
                            metrics[name][sub_name] = result[0] if result else 0
                    else:
    ...
    def collect_mysql_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            queries = {
                ...
                'query_performance': """
                    SELECT 
                        AVG(SUM_TIMER_WAIT/1000000000000) as avg_query_time,
                        MAX(SUM_TIMER_WAIT/1000000000000) as max_query_time,
            ...
            if database_type != 'mysql':
                ...
                if 'cpu_usage' in metrics:
                    ...
                    """, (
                        ...
                        metrics.get('resource_usage', (0, 0, 0))[0],
                        metrics.get('resource_usage', (0, 0, 0))[1], 
                        metrics.get('resource_usage', (0, 0, 0))[2],
    ...
    def _store_in_mongodb(self, database_type: str, metrics: Dict[str, Any]) -> bool:
        """Store metrics in MongoDB using proper pymongo methods"""
        try:
            client = self.connections['mongodb']
            db = client['dbcostops_monitoring']
            ...
            # Insert using proper pymongo method
            db.metrics.insert_one(document)
    ...
    def _store_in_postgresql(self, database_type: str, metrics: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Execute real data insertion function (PostgreSQL uses SELECT, not callproc)
            cursor.execute(
                "SELECT insert_real_metrics(%s, %s, %s::jsonb)",
                [
                    f'{database_type}-primary',
                    database_type,
                    json.dumps(metrics)
