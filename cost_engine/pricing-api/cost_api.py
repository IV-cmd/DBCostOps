...
async def list_regions():
    ...
    try:
        regions = [
            ...
            for code, data in pricing_db.pricing_data["regions"].items()
        ]
...
async def list_instances(database_type: str):
    ...
    try:
        if database_type not in pricing_db.pricing_data.get("database_overheads", {}):
            raise HTTPException(status_code=404, detail="Database type not found")
        
        instances = [
            {"type": itype, **config}
            for itype, config in pricing_db.pricing_data.get("instance_types", {}).items()
        ]
