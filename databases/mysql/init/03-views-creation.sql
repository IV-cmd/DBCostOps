...
FROM cost_metrics 
WHERE timestamp >= DATE_SUB(NOW(), INTERVAL 30 DAY)
GROUP BY DATE(timestamp), metric_name, node_name
ORDER BY date DESC, metric_name;

SELECT 'MySQL views creation completed successfully!' as status;
SELECT 'Views created: v_current_cost_summary, v_optimization_summary, v_backup_summary, v_query_performance_summary, v_resource_usage_summary, v_cost_trends' as details;
